...
angular.module('mainAppCtrls') //monitor控制器
    .controller('monitorCtrl', ['$scope', '$timeout', 'getService', '$stateParams',
        function($scope, $timeout, getService, $stateParams) {
            var vm = $scope.vm = {};
            ...
            //添加警示框
            //vm.addAlert("alert_success", "正在加载...");
            ...
            var clusterName = vm.clusterName+"-member-1_";
            //--- 折柱 ---
            ...
            myChart.showLoading({text: '图二正在努力加载中...'});
            myChart2.showLoading({text: '图一正在努力加载中...'});
            myChart3.showLoading({text: '图三正在努力加载中...'});
            myChart4.showLoading({text: '图四正在努力加载中...'});
            myChart5.showLoading({text: '图五正在努力加载中...'});
            myChart6.showLoading({text: '图六正在努力加载中...'});
            myChart7.showLoading({text: '图七正在努力加载中...'});
            myChart8.showLoading({text: '图八正在努力加载中...'});
            myChart9.showLoading({text: '图九正在努力加载中...'});
            myChart10.showLoading({text: '图十正在努力加载中...'});
            //定义装着time和value的数组
            var cpuLoadtimes = [];
            var cpuLoadvalues = [];
            ...
            vm.monitor = function() {
                ...
                getService.getServiceResult("rds/v1/mysql/clusters/"+vm.clusterId+"/monitor/"+clusterName,angular.fromJson(num))
                    .then(function(data, status, headers, config) {
                        var original = data.data.data;
                        console.log(data.data.data);
                        console.log(angular.fromJson(data.data.data).host);
                        if (angular.fromJson(data.data.data).host.graph==null) {
                            vm.addAlert("alert-warning", "暂无监控数据！");
                            console.log("暂无监控数据！");
                        } else {
                        ...
                        console.log("监控获取失败");
                        console.log(data);
                        vm.addAlert("alert-warning", "监控数据加载失败！");
                    });
            };
            vm.monitor();
